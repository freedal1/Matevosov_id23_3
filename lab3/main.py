...
class Wave:
    ...
    def get_y_values(self, x): #вертикальное положение волны
...
class Float:
    ...
    def calculate_buoyant_force(self): #сила Архимеда
        return self.density_water * self.volume * self.g * self.archimedes_factor

    def update_position(self, waves, dt): #вертикальная позиция поплавка
...
def modify_float_properties(manager, float_obj): #окно для изменения параметров поплавка
...
def draw_info(screen, font, waves, floats): #отображение параметров на экран
    y_offset = 10
...
def main(): #загрузка файла
    ...
    while running:
        ...
        #обновление поззиций волн и поплавков
        for wave in waves:
        ...
        for x in range(800):
            for wave in waves:
                ...
                pygame.draw.circle(screen, (0, 0, 255), (x, int(wave_y)), 3) #цвет поплавка

        for float_obj in floats:
            pygame.draw.circle(screen, (255, 0, 0), (float_obj.x, int(float_obj.y)), 18) #цвет волны
